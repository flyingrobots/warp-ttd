...
| **effect emission** | A side effect produced by a tick transition (diagnostic, notification, export, etc.). Emissions are protocol-level records of what happened, not the delivery itself. Protocol type: `EffectEmissionSummary`. | |
