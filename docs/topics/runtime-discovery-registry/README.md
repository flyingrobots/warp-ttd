...
This shelf owns the local runtime registry and runtime discovery read model used by `discover --json` and `warp_ttd.inspect_runtime_discovery`.
...
| What it does not own | MCP transport wiring, runtime endpoint consent/auth, runtime admission, authority issuance, or mutable runtime control. |
...
| What changes propagate | source priority, redaction, discovery posture, or obstruction changes affect CLI discovery output and MCP parity. |
...
| Runtime hello cannot be composed | `RUNTIME_HELLO_*` reason in discovery record | automation cannot proceed to runtime-specific inspection | inspect target descriptor and runtime hello posture together | `test/cliRuntimeDiscovery.spec.ts`, `test/mcpAdmissionChainSurface.spec.ts` |
...
| Used by | `discover --json`, `warp_ttd.inspect_runtime_discovery`, and runtime discovery goalpost closeout. |
| Cross-shelf impact | Runtime registry changes can alter target enumeration, runtime hello inputs, CLI parity, and MCP parity. |

## Evidence

- Normative claims are in `test-plan.md` rows `R-RDR-1` through `R-RDR-9`.
- Primary sources: `src/app/runtimeRegistry.ts` and `src/app/runtimeDiscovery.ts`.
- Behavior evidence: `test/runtimeRegistry.spec.ts`, `test/cliRuntimeDiscovery.spec.ts`, and `test/mcpAdmissionChainSurface.spec.ts`.
